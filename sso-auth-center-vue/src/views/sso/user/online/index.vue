<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" border :data="pageList">
      ...
      <el-table-column label="操作" fixed="right" align="center" width="180" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            v-hasPermission="['user:online:listOnlineSys']"
            size="mini"
            type="text"
            icon="el-icon-view"
            @click="handleShowSysOnline(scope.row)"
          >已登子系统
          </el-button>

          <el-button
            v-if="scope.row.supperAdminFlag!==true"
            v-hasPermission="['user:online:retreatUser']"
            size="mini"
            type="text"
            icon="el-icon-delete-solid"
            @click="retreatUser(scope.row)"
          >强退
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="已登录的子系统" :visible.sync="sysOnlineOpen">
      <el-table v-loading="loading" border :data="sysOnlineList" :show-overflow-tooltip="true">
...
export default {
  ...
  methods: {
    ...
    buildTimeRange() {
      ...
      if (this.loginDateRange != null && this.loginDateRange !== '') {
