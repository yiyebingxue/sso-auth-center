  ...
  <div class="platform-detail-container">
    <div class="platformClass">
      ...
      <el-row :gutter="10">
        ...
        <el-col v-if="hasUpdatePermission" :md="3" :lg="3">
          <sysIcon v-if="detailData.sysIcon !=='' && detailData.sysIcon !==undefined" :sys-id="detailData.sysId" :img-url="detailData.sysIcon" />
          <sysIcon v-else :sys-id="detailData.sysId" :img-url="detailData.sysDefaultIcon" />
        </el-col>
        <el-col v-if="!hasUpdatePermission" :md="3" :lg="3">
          <img
            v-if="detailData.sysIcon !=='' && detailData.sysIcon !==undefined"
            :src="detailData.sysIcon"
  ...
  methods: {
    getDetail() {
      getDetailBySysCode(this.targetSysCode).then((response) => {
        ...
        // 处理默认图片地址
        if (this.detailData.sysIcon == null || this.detailData.sysIcon === '') {
